...
async function simulateUpdate() {
  ...
  const { data: settings } = await (supabase as any).from('therapist_settings').select('therapist_id').limit(1).single()
  ...
  if (!client) {
    ...
    const { data: newApp, error: appError } = await (supabase as any).from('appointments').insert({
    ...
    if (appError) {
      ...
      console.log('🔄 Updating appointment status to FINALIZATĂ...')
      await supabase.from('appointments').update({ status: 'FINALIZATĂ' }).eq('id', newApp.id)
