...
import { ArrowUpRight, Building2, FileText, Home, ShieldAlert, Video } from "lucide-react";
...
export function AppointmentRow({
  ...
  return (
    <div className={cn(
      "rounded-[1.6rem] border border-border/60 px-4 py-4 transition-all hover:-translate-y-0.5 hover:shadow-sm",
      isDuty ? "border-dashed bg-muted/30" : "bg-card",
    )}>
      <div className="flex items-start gap-4">
        <div className="w-16 shrink-0 rounded-[1.2rem] border border-border/60 bg-muted/20 px-2 py-3 text-center">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-1.5 flex items-center gap-3 text-[11px] font-medium text-muted-foreground flex-wrap">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {appointment.hasInvoice !== undefined ? (
                <Badge variant={appointment.hasInvoice ? "info" : "outline"} className="normal-case tracking-normal">
                  {appointment.hasInvoice ? "Factură emisă" : "Facturare în așteptare"}
                </Badge>
              ) : null}
            </div>
          ) : null}
          ...
          {showClinicalContext && !isDuty ? (
            <div className="mt-4 flex flex-wrap gap-2">
              <Link
                ...
                className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-muted/20 px-3 py-1.5 text-xs font-semibold text-foreground transition-colors hover:bg-muted/40"
              >
                Deschide
